...
graph TB
    subgraph "КУБА"
        ...
        subgraph "Центральный офис - Гавана"
            ...
            MON[Сервер мониторинга<br/>Zabbix + Grafana<br/>192.168.100.50]
...
│  │ 3U  │                                                 │       │
│  ├──────────────────────────────────────────────────────┤       │
│  │ 4U  │ Сервер мониторинга (Ubuntu 22.04 LTS)         │       │
...
graph TB
    ...
    subgraph "Центральный офис - Гавана"
        ...
        MON["📊 Сервер мониторинга<br/>192.168.100.50/24<br/>Zabbix + Grafana<br/>VPN Server"]
...
| **Офис Гавана** | Сервер мониторинга | 192.168.100.50 | /24 | 192.168.100.1 | 8.8.8.8 | Zabbix/Grafana |
...
sequenceDiagram
    ...
    participant SNMP as Сервер мониторинга<br/>(SNMP/Syslog)
    ...
    Note over SNMP: Grafana визуализация<br/>Zabbix алертинг
```
...
graph TB
    ...
    subgraph "Центральный офис - Гавана"
        subgraph "Сервер мониторинга 192.168.100.50"
            ZABBIX[Zabbix Server<br/>Сбор метрик SNMP<br/>Получение Syslog<br/>Алертинг]
            GRAFANA[Grafana<br/>Визуализация<br/>Дашборды]
...
│  │  ├─ Источник: Сервер мониторинга (192.168.100.50)          ││
│  │  ├─ Назначение: Quantum GM (192.168.X.10)                   ││
│  │  ├─ Протокол: UDP                                            ││
│  │  ├─ Порт: 161                                                ││
│  │  ├─ Действие: РАЗРЕШИТЬ                                     ││
...
│  │  ├─ Назначение: Сервер мониторинга (192.168.100.50)        ││
│  │  ├─ Протокол: UDP                                            ││
│  │  ├─ Порт: 514                                                ││
│  │  ├─ Действие: РАЗРЕШИТЬ                                     ││
...
│  ├─ 10.100.0.50  - Сервер мониторинга - локально                │
...
│  ├─ Сервер мониторинга (10.100.0.50)                            │
│  ├─ Веб-интерфейсы Grafana/Zabbix (HTTPS)                       │
...
- **🔵 Голубой (#87CEEB)** - Серверы мониторинга
...
**Документ подготовлен:** 12 октября 2025  
**Версия:** 1.0  
